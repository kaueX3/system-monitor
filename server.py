...
API para coleta de métricas e status de endpoints
Deploy: railway.app
"""
...
def register_endpoint():
    """Registra novo endpoint/máquina"""
    try:
        data = request.json
        ...
        print(f"✅ Novo endpoint registrado: {endpoints[endpoint_id]['hostname']} ({endpoint_id})")
...
def receive_metrics():
    ...
    try:
        ...
        print(f"� {len(metrics)} métricas recebidas de {endpoint_id}")
...
def receive_screenshot():
    """Recebe captura de tela para monitoramento remoto"""
    try:
        data = request.json
        endpoint_id = data.get('endpoint_id', 'unknown')
...
def get_endpoints():
    """Retorna lista de todos os endpoints"""
...
def get_screenshot(endpoint_id):
    """Retorna screenshot de um endpoint específico"""
...
<head>
    ...
    <title>LEALDADE - Painel Central</title>
    <style>
...
<body>
    ...
    <div class="update-time">
        ...
        <div>Última atualização: {{UPDATE_TIME}}</div>
    </div>
